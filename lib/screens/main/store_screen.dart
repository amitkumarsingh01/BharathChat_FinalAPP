...
class _StoreScreenState extends State<StoreScreen> {
  ...
  void _processPurchase(Map<String, dynamic> item) async {
    try {
      ...
      final response = await http.post(
        ...
        body: json.encode({'user_id': userId, 'shop_id': shopId}),
      );
      ...
      if (response.statusCode == 200) {
        ...
        final merchantOrderId = data['merchant_order_id'];
        final diamondAmount = data['diamond_amount'];
        ...
        if (redirectUrl != null && merchantOrderId != null) {
          // Store the current order ID and start monitoring
          _currentOrderId = merchantOrderId;
          _startPaymentStatusMonitoring(merchantOrderId, diamondAmount);
          ...
          print('No redirect URL or merchant order ID received in response');
          throw Exception('No redirect URL or merchant order ID received');
        }
      } else {
  ...
  void _startPaymentStatusMonitoring(String merchantOrderId, int diamondAmount) {
    // Cancel any existing timer
    _paymentStatusTimer?.cancel();
    
    // Start polling for payment status every 5 seconds
    _paymentStatusTimer = Timer.periodic(const Duration(seconds: 5), (timer) {
      _checkPaymentStatus(merchantOrderId, diamondAmount, timer);
    });
  }

  Future<void> _checkPaymentStatus(String merchantOrderId, int diamondAmount, Timer timer) async {
    try {
      print('Checking payment status for order: $merchantOrderId');
      
      final response = await http.post(
        Uri.parse('https://server.bharathchat.com/order-status'),
        headers: {
          'accept': 'application/json',
          'Content-Type': 'application/json',
        },
        body: json.encode({'merchant_order_id': merchantOrderId}),
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (state == 'COMPLETED') {
          ...
          await _creditDiamondsToUser(diamondAmount);
          
          // Show success message
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text('Payment successful! $diamondAmount diamonds credited to your account.'),
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text('Payment $state. Please try again.'),
                backgroundColor: Colors.red,
              ),
            );
          }
        }
        // If state is still PENDING, continue monitoring
      } else {
        print('Failed to check payment status: ${response.statusCode}');
      }
    } catch (e) {
      print('Error checking payment status: $e');
      // Don't cancel timer on error, continue monitoring
    }
  }

  Future<void> _creditDiamondsToUser(int diamondAmount) async {
    try {
      // This would typically be handled by the backend when payment is completed
      // But we can also make an API call to update the user's diamond balance
      // You may need to implement this API endpoint on your backend
      
      print('Crediting $diamondAmount diamonds to user');
      
      // Option 1: If you have an API endpoint to credit diamonds
      // final response = await http.post(
      //   Uri.parse('https://server.bharathchat.com/credit-diamonds'),
      //   headers: {
      //     'accept': 'application/json',
      //     'Content-Type': 'application/json',
      //   },
      ...
      //     'merchant_order_id': _currentOrderId,
      //   }),
      // );
      
      // Option 2: Just refresh user data (assuming backend handles the crediting)
      _loadUserData();
      
    } catch (e) {
      print('Error crediting diamonds: $e');
    }
  }
