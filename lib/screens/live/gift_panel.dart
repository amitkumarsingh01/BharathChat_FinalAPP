...
class _GiftPanelState extends State<GiftPanel> {
  ...
  Future<void> _loadGifts() async {
    try {
      ...
      // Add more gifts if not present (roses, champagne, etc.)
      final List<Map<String, dynamic>> extraGifts = [
        {
          'id': 1001,
          'name': 'Rose',
          'diamond_amount': 5,
          'gif_filename': 'rose.gif',
        },
        {
          'id': 1002,
          'name': 'Champagne',
          'diamond_amount': 50,
          'gif_filename': 'champagne.gif',
        },
        {
          'id': 1003,
          'name': 'Teddy Bear',
          'diamond_amount': 20,
          'gif_filename': 'teddy.gif',
        },
        {
          'id': 1004,
          'name': 'Car',
          'diamond_amount': 100,
          'gif_filename': 'car.gif',
        },
      ];
      // Only add if not already present by name
      for (final g in extraGifts) {
        if (!giftsData.any((gift) => gift['name'] == g['name'])) {
          giftsData.add(g);
        }
      }
      setState(() {
