...
class _WithdrawDiamondState extends State<WithdrawDiamond> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Failed to submit request'), backgroundColor: Colors.red));
    } finally {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _isLoading ? const Center(child: CircularProgressIndicator(color: Colors.orange)) : _buildBody(),
    );
  }
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          _withdrawals.isEmpty ? const Text('No withdrawal requests yet.', style: TextStyle(color: Colors.white70)) : _buildWithdrawalsList(),
        ],
      ),
    );
  }
        ...
        children: [
          TextFormField(
            ...
            validator: (value) {
              ...
              if (_userDiamonds != null && n > _userDiamonds!) return 'You only have $_userDiamonds';
              return null;
            },
          ...
          if (_enteredDiamonds >= min)
            Text('You will receive: ${(_enteredDiamonds * rate).toStringAsFixed(2)}', style: const TextStyle(color: Colors.white70)),
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              ...
              child: _submitting ? const CircularProgressIndicator(color: Colors.white) : const Text('Request Withdrawal', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.white)),
            ),
          ),
        ],
      ),
